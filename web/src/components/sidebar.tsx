...
export function Sidebar({ className, ...props }: React.ComponentPropsWithoutRef<'nav'>) {
  return (
    <nav
      ...
      className={clsx(
        className,
        'flex h-full min-h-0 flex-col bg-zinc-50 dark:bg-zinc-900/50 border-r border-zinc-950/5 dark:border-white/5'
      )}
    />
  )
}
...
export const SidebarItem = forwardRef(function SidebarItem(
  ...
  let classes = clsx(
    ...
    // Leading icon/icon-only - strictly outline for Lucide
    '*:data-[slot=icon]:size-6 *:data-[slot=icon]:shrink-0 *:data-[slot=icon]:fill-none *:data-[slot=icon]:stroke-current *:data-[slot=icon]:text-zinc-500 sm:*:data-[slot=icon]:size-5',
